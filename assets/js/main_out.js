(function (wHandle, wjQuery) {
  ...
  if (null !== wHandle.localStorage) {
    wjQuery(window).load(function () {
      ...
      wjQuery(".save").change(function () {
        var id = wjQuery(this).data("box-id");
        var value =
          id == 0 ? wjQuery(this).val() : wjQuery(this).prop("checked");
  ...
  wjQuery.ajax({
    ...
    success: function (data) {
      var stamp = Date.now();
      response = JSON.parse(data.names);
      for (var i = 0; i < response.length; i++) knownHats[response[i]] = stamp;
      for (var i in knownHats) if (knownHats[i] !== stamp) delete knownHats[i];
    },
  });
  ...
  function drawStats() {
    ...
    var rows = [
      ...
      `${(stats.info.update * 2.5).toFixed(1)}% load @ ${prettyPrintTime(
        stats.info.uptime
      )}`,
    ];
    var width = 0;
  ...
  function drawMinimap() {
    ...
    var myPosY =
      beginY + ((cameraY + border.height / 2) / border.height) * height;
    ...
    if ((oldcameraX, oldcameraY)) {
      var myPosX1 =
        beginX + ((oldcameraX + border.width / 2) / border.width) * width;
      var myPosY1 =
        beginY + ((oldcameraY + border.height / 2) / border.height) * height;
  ...
  getVipSkinsFrames = function (a) {
    ...
    for (var i = 0; i < skinArray.length; i++) {
      if (
        skinArray[i].skinType == "vip" &&
        skinArray[i].productId == skinName
      ) {
        knownVipSkins[a] = [];
        knownVipSkins[a].image = [];
        for (j = 0; j < skinArray[i].image.length; j++) {
          knownVipSkins[a].image[j] = new Image();
          knownVipSkins[a].image[
            j
          ].src = `${SKIN_URL}${skinArray[i].productId}/${skinArray[i].image[j]}.png`;
        }
  ...
  Cell.prototype = {
    ...
    setHats: function (value) {
      ...
      if (
        this.hats === null ||
        !knownHats.hasOwnProperty(this.hats) ||
        loadedHats[this.hats]
      )
        return;
    ...
    drawShape: function (ctx) {
      ctx.fillStyle =
        settings.showColor && settings.showSkins
          ? this.skinColor
          : settings.showColor && !settings.showSkins
          ? this.color
      ...
      if (!this.ejected && 20 < this.s) ctx.stroke();
      ctx.fill();
  ...
  function init() {
    ...
    function handleScroll(event) {
      ...
      mouseZ *= Math.pow(0.9, event.wheelDelta / -120 || event.detail || 0);
      1 > mouseZ && (mouseZ = 1);

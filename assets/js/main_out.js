(function(wHandle, wjQuery) {
    ...
    var wsUrl = null,
        SKIN_URL = "./skins/",
        HATS_URL = "./assets/hats/",
        url = "./assets/js/",
        configFile = "GameConfiguration.json",
        config = JSON.parse(httpGet(url + configFile)),
        USE_HTTPS = "https:" == wHandle.location.protocol,
        PI_2 = Math.PI * 2,
        SEND_254 = new Uint8Array([254, 6, 0, 0, 0]),
        SEND_255 = new Uint8Array([255, 1, 0, 0, 0]),
        UINT8_CACHE = { //unit module
            1: new Uint8Array([1]),
    ...
    function wsOpen() {
        ...
        wsSend(SEND_254);
        wsSend(SEND_255);
        wjQuery("#level-blocker").fadeOut(300);
        wjQuery("#shop-blocker").fadeOut(300);
    ...
    function wsClose(e) {
        ...
        wsCleanup();
        gameReset();
        wjQuery("#level-blocker").fadeIn(300);
        wjQuery("#shop-blocker").fadeIn(300);
        setTimeout(function() {
        ...
        switch (packetId) {
            case 0x10: // update nodes
                var killer, killed, id, node, x, y, s, flags, cell,
                    updColor, updName, updSkin, count, color, strokecolor, name, skin;
    ...
    function sendPlay(name) {
        skinname = wjQuery("#skin").val();
        if (skinname) name = skinname + name;
    ...
    wjQuery.ajax({
        ...
        success: function(data) {
            ...
            response = JSON.parse(data.names);
            for (var i = 0; i < response.length; i++)
                knownHats[response[i]] = stamp;
            for (var i in knownHats)
                if (knownHats[i] !== stamp) delete knownHats[i];
        }
    });
    ...
    function drawStats() {
        ...
        var rows = [
            ...
            `${stats.info.minion} minion`,
            `${stats.info.playerID} playerID`,
        ...
        for (var i = 0; i < rows.length; i++)
            ctx.fillText(rows[i], 2, 0 + i * (14 + 2));
    }
    ...
    function drawLeaderboard() {
        ...
        ctx.globalAlpha = 1; //gradian color leaderboard
        // var gradient = ctx.createLinearGradient(0, 0, 150, 100);
        // gradient.addColorStop(0, "#0004FF");
        ...
        if (leaderboard.type === "pie") {
            ...
            for (var i = 0; i < len; i++) {
                ...
                gradient.addColorStop(0, "#0004FF");
                gradient.addColorStop(1, "#8000FF");
                ctx.fillStyle = gradient; name gradient name on leaderborad!!*/
                ctx.fillStyle = IslbColor(text) ? getlbColor(text, ctx) : isMe ? "#FAA" : "#FFF";
    ...
    var skinArray = config["skins"];knownSkinColor
    var lbColorArray = config["leaderboard_colors"];

    getSkinColor = function(a) {
		if(knownSkinColor[a] != undefined) return knownSkinColor[a];
        var skinName = a;
        var color = "";
        for (var i = 0; i < skinArray.length; i++) {
            if (skinArray[i].productId == skinName) {
                color = skinArray[i].cellColor;
                knownSkinColor[a] = color;
				return knownSkinColor[a]
            }
        }
    }
    getVipSkinsFrames = function(a) {
		if(knownVipSkins[a] != undefined) return knownVipSkins[a];
        var skinName = a;
        for (var i = 0; i < skinArray.length; i++) {
            if (skinArray[i].skinType == "vip" && skinArray[i].productId == skinName) {
                knownVipSkins[a] = []; knownVipSkins[a].image = []; for(j=0; j<skinArray[i].image.length; j++) {
					knownVipSkins[a].image[j] = new Image();
					knownVipSkins[a].image[j].src = `${SKIN_URL}${skinArray[i].productId}/${skinArray[i].image[j]}.png`;
				}
				return knownVipSkins[a];
            }
        }
    }
    
    getSkinByFrame = function(a,i) {
		if(knownVipSkins[a] != undefined) return knownVipSkins[a].image[i];
	}
	
    getlbColor = function(a, ctx) {
		if(knownLbColor[a] != undefined) return knownLbColor[a].LbColor;
        var Name = a;
        var gradient = ctx.createLinearGradient(0, 0, 150, 10);
        for (var i = 0; i < lbColorArray.length; i++) {
            if (lbColorArray[i].name == Name) {
				knownLbColor[a] = [];
                gradient.addColorStop(0, lbColorArray[i].color1);
                gradient.addColorStop(1, lbColorArray[i].color2);
				knownLbColor[a].LbColor = gradient;
                return knownLbColor[a].LbColor
            }
        }
    }

    IslbColor = function(a) {
		if(knownLbColor[a] != undefined) return true;
        var Name = a;
		for (var i = 0; i < lbColorArray.length; i++) {
            if (lbColorArray[i].name == Name) {
                return true
    ...
    function Cell(id, x, y, s, name, color, skin, flags) {
        ...
        skin ? this.setColor(color, getSkinColor(skin)) : this.setColor(color);
        this.setName(name);
    ...
    Cell.prototype = {
        ...
        destroy: function(killerId) {
            ...
            if (cells.mine.remove(this.id) && cells.mine.length === 0)
                console.log(stats.maxScore), showESCOverlay(), oldcameraX = cameraX, oldcameraY = cameraY;
            this.destroyed = true;
        ...
        setHats: function(value) {
            this.hats = value;
            if (this.hats === null || !knownHats.hasOwnProperty(this.hats) || loadedHats[this.hats])
                return;
            loadedHats[this.hats] = new Image();
            loadedHats[this.hats].src = `${HATS_URL}${this.hats}.png`;
        },
        setColor: function(value, skinColor) {
            if (!value) {
                log.warn("got no color");
                return;
            }
            skinColor ? this.skinColor = skinColor : this.skinColor = value;
            this.color = value;
            this.sColor = skinColor ? darkenColor(skinColor) : darkenColor(value);
        },
        draw: function(ctx) {
            ctx.save();
            this.drawShape(ctx);
        ...
        drawShape: function(ctx) {
            ctx.fillStyle = settings.showColor && settings.showSkins ? this.skinColor :
                settings.showColor && !settings.showSkins ? this.color :
                Cell.prototype.color;
            ctx.strokeStyle = settings.showColor && settings.showSkins ? this.sColor :
                settings.showColor && !settings.showSkins ? darkenColor(this.color) :
                Cell.prototype.color;
        ...
        drawHat: function(ctx) {
            if (settings.showHats && this.s > 55 && this.hats && this.name !== null && this.name !== "minion") {
                var hats = loadedHats[this.hats];
                if (hats && hats.complete && hats.width && hats.height) {
                    ctx.save();
                    ctx.globalAlpha = .9;
                    ctx.drawImage(hats, this.x - this.s, this.y - this.s - this.s, 2 * this.s, 2 * this.s);
                    ctx.restore();
                }
            }
        ...
        drawText: function(ctx) {
            if (this.s < 55 || this.jagged || this.name == "minion") return;
        ...
        wHandle.onkeydown = function(event) {
            switch (event.keyCode) {
                ...
                case 81: // Q
                    ...
                    pressed.q = true;
                    if (!escOverlayShown) minionControlled = !minionControlled;
                    break;
                case 69: // E
    ...
    wHandle.setGird = function(a) {
        settings.showGrid = !a;
    };
    wHandle.setHats = function(a) {
        settings.showHats = !a;
    };
    ...
    wHandle.play = function(a) {
        ...
        hideESCOverlay();
        /*setTimeout(function() {
        wsSend(UINT8_CACHE[15]);//extra mass packet
        wsSend(UINT8_CACHE[14]);//bots packet
        }, 1000); //ExtraMass for login user!*/
        setTimeout(function() {
            var stats = {
                "bots": [{
                    "hasbots": "yes",
            ...
            var writer = new Writer(true);
            writer.setUint8(0x0D);
            writer.setStringUTF8(json);
            wsSend(writer);
        }, 1000); //ExtraMass and bots json array for login user!
    };
    wHandle.openSkinsList = function() {
        ...
        wjQuery.get("include/gallery.php").then(function(data) {
            wjQuery("#inPageModalTitle").hide();
            wjQuery("#pills-tab").show();
            wjQuery("#inPageModalTitle").text("Skins");
            wjQuery("#inPageModalBody").html(data);
        });
    };
    wHandle.setSkinToDB = function(a) {
        console.log(a);
        wjQuery("#skin").val("{" + a + "}");
        wjQuery("#skinLabel").text("");
        wjQuery(".skinUrl").attr("src", `${SKIN_URL}${a}.png`);
    };
    wjQuery('.modal').on('hide.bs.modal', function(e) {
        wjQuery("#overlays").removeClass("blurpanel");
    });
    wjQuery('.modal').on('show.bs.modal', function(e) {
        wjQuery("#overlays").addClass("blurpanel");
    });
    wjQuery("#customFile").change(function() {
        wjQuery(".custom-file-label").text(this.files[0].name);
    });
    wHandle.onload = init;

...
function PacketHandler(gameServer, socket) {
    ...
    this.socket = socket;
	this.player = this.socket.playerTracker;
    this.protocol = 0;
...
PacketHandler.prototype.handshake_onCompleted = function (protocol, key) {
    this.handler = {
		...
		13: this.message_onPlayer.bind(this),
		14: this.message_onPremiumBots.bind(this),
		15: this.message_onExtraMass.bind(this),
...
PacketHandler.prototype.message_onJoin = function (message) {
	...
	//if(this.gameServer.config.serverGamemode == 1) text=" ";
    this.setNickname(text);
};

PacketHandler.prototype.message_onPlayer = function (message) {
	/*var json = JSON.parse(Buffer.from(texts, 'base64').toString());
	console.log(json);
	if(json.bots[0].hasbots == "yes") this.message_onPremiumBots(this.player, json.bots[0].number);
	if(json.mass[0].hasmass == "yes") this.message_onExtraMass(this.player, json.mass[0].massstart);*/
    var reader = new BinaryReader(message);
    reader.skipBytes(1);
    var texts = null;
    if (this.protocol < 6)
        texts = reader.readStringZeroUnicode();
    else
        texts = reader.readStringZeroUtf8();
...
PacketHandler.prototype.message_onExtraMass = function (player, massstart) {
	if (!player.cells.length) return;
	var size = Math.sqrt(massstart * 100);
	var PlayerName = player._name;
	if (/\{([\w]+)\}/.test(PlayerName)) {
		PlayerName = PlayerName.replace(/\s*\{.*?\}\s*/g, '');
	}
    for (var i in this.gameServer.clients) {
                var client = this.gameServer.clients[i].playerTracker;
                if (client.pID == player.pID && !player.startingMass) {
                    for (var j in client.cells) {
                        client.cells[j].setSize(size);
                    }
					this.gameServer.sendChatMessage(null, this.socket.playerTracker, "Starting with extra mass for "+PlayerName);
					player.startingMass = true;
                    break;
                }
            }
};

PacketHandler.prototype.message_onPremiumBots = function(player, number) {
	if (!player.cells.length) return;
    var add = number ? number : 20;
...
PacketHandler.prototype.setNickname = function (text) {
    ...
    if(this.gameServer.config.serverGamemode == 1) skin = null;
